...
void create_syn_packet(struct sockaddr_in *src, struct sockaddr_in *dst,
  ...
  // checksum, correct calculation follows later
  iph->ip_sum = 0;
  // source address
...
//                         int32_t seq, int32_t ack_seq, char *data, int
//                         data_len, char **out_packet, int *out_packet_len) {
//   // datagram to represent the packet
//   char *datagram = calloc(DATAGRAM_LEN, sizeof(char));
//
//   // required structs for IP and TCP header
...
int main(int argc, char **argv) {
  ...
  if (setsockopt(sock, SOL_SOCKET, SO_BROADCAST, (const char *)&one,
                 sizeof(int)) != 0) {
  ...
  // //  call bind with port number specified as zero to get an unused source
  // port if (bind(sock, (struct sockaddr *)&saddr, sizeof(struct sockaddr)) ==
  // -1) {
  //   printf("bind() failed\n");
  //   return 1;
  // }
  ...
  // &packet_len); if ((sent = sendto(sock, packet, packet_len, 0, (struct
  // sockaddr *)&daddr,
  //                    sizeof(struct sockaddr))) == -1) {
  //   printf("sendto() failed\n");
  // } else {
  ...
  //   create_ack_packet(&saddr, &daddr, ack_num, new_seq_num, &packet,
  //                     &packet_len);
  //   if ((sent = sendto(sock, packet, packet_len, 0, (struct sockaddr
  //   *)&daddr,
  //                      sizeof(struct sockaddr))) == -1) {
  //     printf("send failed\n");
  //   } else {
